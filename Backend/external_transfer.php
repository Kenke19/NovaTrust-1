...
// MOCK MODE FLAG
$mock_paystack = true; // Set to false when you want to use real Paystack
...
if ($mock_paystack) {
    ...
    $recipient_code = "RCP_" . uniqid();
    // Simulate transfer result (success if amount < 5000000, fail otherwise)
    if ($amount < 5000000) {
        $transfer_result = [
            "status" => true,
            "message" => "Transfer simulated successfully",
            "data" => [
                "reference" => "SIM_TEST_" . uniqid(),
                "amount" => $amount * 100,
        ...
        logTransferError($transaction_id, "Simulated transfer failed: amount too high");
        http_response_code(400);
        echo json_encode(['error' => 'Transfer failed', 'details' => 'Simulated failure: amount too high']);
        exit();
    }
} else {
    // --- REAL PAYSTACK LOGIC (unreachable while $mock_paystack is true) ---
    $recipient_payload = [
        "type" => "nuban",
        "name" => "Test User",
